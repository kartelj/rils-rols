...
class RILSROLSEnsembleRegressor(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        best_model, best_model_simp, _, _, _ = results[0]
        best_fit = best_model.fitness(X, y, False)
        for model, model_simp, fit_calls, main_it, ls_it in results:
            self.fit_calls+=fit_calls
            self.main_it+=main_it
            self.ls_it+ls_it
    ...
    def fit_report_string(self, X, y):
        ...
        return "maxTime={0}\tmaxFitCalls={1}\tseed={2}\tsizePenalty={3}\tR2={4:.7f}\tRMSE={5:.7f}\tsize={6}\tsec={7:.1f}\tmainIt={8}\tlsIt={9}\tfitCalls={10}\texpr={11}\texprSimp={12}\terrTol={13}".format(
            self.max_seconds,self.max_fit_calls,self.random_state,self.complexity_penalty, 1-fitness[0], fitness[1], self.complexity(), self.time_elapsed, self.main_it, self.ls_it,self.fit_calls, self.model, self.model_simp, self.error_tolerance)

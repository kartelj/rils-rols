...
\section{Proposed iterated local search method}\label{sec:rils}
   ...
   In the literate, one of the mostly applied ILS versions in the literature is pseudo-coded in Algorithm~\ref{alg:ils}. Note that we also used the core of this version in our \textsc{Risl-Rols} algorithm. \fxnote{Provjeriti da li je ovo tačno -- popraviti algoritam ako nije sa onim što je korišten...}
   
   \begin{algorithm}
  		\begin{algorithmic}[1] 
  			 \State \textbf{Input}: a COP problem; an LS procedure  
  	 		...
  	 		\State  $s \gets LS(s)$
   	 		\While{\emph{Stopping criteria is not met}}
   	    		 \State  $s' \gets$ Perturbation($s$)
   	    		 \State  $s' \gets$ LS($s'$)
   	    		 \State  $ s \gets$ AcceptanceCriterion($s, s'$)
   	 		\EndWhile
   		\end{algorithmic}
    	\caption{An ILS framework from the literature.}
    	   	     \label{alg:ils}
    \end{algorithm}  
   
An initial solution may be generated randomly or by using a greedy heuristic, which is afterwards, improved by LS procedure. At each iteration, ILS applies three steps. First, the current incumbent solution is perturbed, that is mutated. Secondly, the perturbed solution is then improved by LS procedure. Thirdly, the LS outcome is thereafter compared to the incumbent solution and possibly replaced as the new one, with accordance to the chosen acceptance criterion. 

...
The rate of success of \textsc{Rils}--\textsc{Rols} algorithm seems to slightly increase as the expected number of variables gets larger in case of the data with no noise;
	the increase is significant in the case of a higher noise in data. For example, the lowest percentages of success for \textsc{Rils}-\textsc{Rols} algorithm are noticed for noisy data where exact models consist of just one or two variables. It seems that in these cases the algorithm is more likely in favor of including additional variables at the cost of increasing model accuracy  in order to improve the fitness value over iterations. In the presence of zero level od noise, OLS method will find much easier a smaller model (on $\approx$ 45\% cases) of a high  accuracy w.r.t.\ the input data. In the next iterations, the method will hardly intend for adding variables in the current best solution at the cost of improving fitness value, as the size of model will contribute more to its increase as the first two product terms in~\ref{eq:fitness} are already small (close to 1).      \fxnote{TODO: need to be further expanded...}
...
\begin{figure}[!ht]
	...
	\label{fig:compExact_noise_varsize}
\end{figure}

...
\emph{Case 1}. 
Subtree $ss$ is the whole tree $s$. Following on the the previous exemplary expression tree, this means that multiplication node that connects $(x+y^3)$ and $x/\sqrt{y}$ is to be changed. For example, multiplication can be replaced by addition, which forms a perturbation $(x+y^3)+ x/\sqrt{y}$. 


\emph{Case 2}. 
Subtree $ss$ has arity of at least 1. This means that the left subtree exists, so the left subtree node is to be changed. 
For example, if $ss=x+y^3$ the overall perturbation might be $(x/y^3)+x/\sqrt{y}$ (addition is replaced by division). 

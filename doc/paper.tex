...
The rate of success of \textsc{Rils}--\textsc{Rols} algorithm seems to slightly increase as the expected number of variables gets larger in case of the data with no noise;
	the increase is significant in the case of a higher noise in data. For example, the lowest percentages of success for \textsc{Rils}-\textsc{Rols} algorithm are noticed for noisy data where exact models consist of just one or two variables. It seems that in these cases the algorithm is more likely favorizing including additional variables at the cost of increasing model accuracy  in order to improve the fitness value over iterations. %Obtaining an accurate model with a few variables in the presence of a high level of nose in the data 

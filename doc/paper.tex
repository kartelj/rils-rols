...
Note that when the algorithm terminates with $R^2=1$ and $RMSE=0$, the match of found model and the exact model is proved -- the results are reported in the second block. However, in case when $R^2 \neq 1$, that is $R^2 \geq 0.999$, we manually check these cases comparing the match of obtained model and the exact model, indicating the result. \fxnote{Aco, projeri ovu recenicu, da nisam promasio...}
...
\begin{itemize}
	...
	\item \fxnote{Marko: opisati rezultate za $R^2 >0.999$}
\end{itemize}

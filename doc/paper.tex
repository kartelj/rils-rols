    ...
    \begin{algorithm}
   	\begin{algorithmic}[1] 
   		...
   		\While{\emph{stopping criteria is not met}}
   			...
   			\State $s_{perturbations} \gets $ \texttt{Generate1Perturbations}($s$)
   			...
   			\Else
   				...
   				\If{$start_{candidates} \cap start_{tried} = \emptyset$} // all 1-perturbations around $bs$ tried
   					\State $s' \gets $ \texttt{RandomPick}($start_{candidates}$)
   					\State $start_{candidates} \gets $ \texttt{Generate1Perturbations}($s'$) 
   				\EndIf

    ...
    \begin{algorithm}
   	\begin{algorithmic}[1] 
   		...
   		\While{\emph{stopping criteria is not met}}
   			...
   			\State $s_{perturbations} \gets $ \texttt{Generate1Perturbations}($s$) \label{line:sPert}
   			...
   			\Else
   				\State $start_{candidates} \gets $ \texttt{Generate1Perturbations}($bs$)
...
As the name of this procedure (\texttt{Generate1Perturbations}) suggests, perturbation is local -- meaning the closeness of starting solution $s$ and any of perturbations is 1 (we call it 1-perturbation sometimes). The precise way of generating perturbation is described in a separate Section~\ref{sec:pertGen}. 

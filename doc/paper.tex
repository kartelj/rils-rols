...
\begin{algorithm}
	...
	\begin{algorithmic}[1] 
		...
		\EndProcedure
	\end{algorithmic}
	\caption{Generation of all 1-perturbations for a given solution.}
	\label{alg:pert}
\end{algorithm}  
...
\begin{algorithm}	
	...
	\begin{algorithmic}[1]
		...
		\EndProcedure
	\end{algorithmic}
	\caption{Generation of 1-perturbations for a given solution around given node.}
	\label{alg:pertNode}
\end{algorithm}
...
\emph{Case 1}. 
Observed node $n$ is the whole tree $s$. Following on the the previous exemplary expression tree, this means that multiplication node that connects $(x+y^3)$ and $x/\sqrt{y}$ is to be changed. For example, multiplication can be replaced by addition, which forms a perturbation $(x+y^3)+ x/\sqrt{y}$. 


\emph{Case 2}. 
Node $n$ has arity of at least 1. This means that the left subtree exists, so the left subtree node is to be changed. 
...
For a given perturbation $p$, local search systematically explores all 1-perturbations around $p$. It relies on the \emph{best-improvement} strategy, meaning that all 1-perturbations (for all subtrees) are considered. Before checking if the candidate solution ($new$) is better that the actual best $bp$, OLS coefficient fitting is performed. 

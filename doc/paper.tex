...
The results are displayed in Figure~\ref{fig:compExact_noise_size}. They are grouped with accordance to the (three) formed sub-groups ($x$-axis) and for each of them the exact percentage rate of success of \textsc{Rils}--\textsc{Rols} algorithm is shown ($y$-axis). As expected, the success rate is the highest for the small--sized instances; it is sightly bellow 90\% for the clean (no-noise) data. For more noisy data, the success rate of the algorithm gets decreased. For example, on the small-sized instances on the noisy data with the level of noise of 0.01, the rate of success is about 35 \%.

...
\section{Introduction}\label{sec:introduction}
    ...
    In this work we present yet another GP--based approach to solve SR, which utilizes the popular iterated local search meta-heuristic~\cite{lourencco2003iterated,lourencco2019iterated}; in the remaining text, we label the metod by \textsc{Rils-Rols}.  The main code of the efficient local search is utilization of the ordinary least square method~\cite{leng2007ordinary} that is in charge for an efficient finding suitable coefficients of linearised formulas.  \fxnote{TODO: maybe expand the core of the algorithm... }
...
\begin{definition}
  Given is a dataset $D = \{(x_i, y_i)\}_{i=1}^n$, where $\textbf{x} \in \mathbb{R}^d$ represents input variables (features), and $y$ target variable. Suppose that there exists an analytical model of the form $f(\textbf{x})= g^*(\textbf{x}, \theta^*) + \epsilon $ that is a generator of all observations in $D$.  
...
Koza~\cite{koza1994genetic} introduced the problem of SR as a specific application of genetic programming. GP deals with the object called programs, which need to be optimized. In particular, the programs are represented by syntax trees consisting of functions/operations over input features and constants. \fxnote{TODO: dati primjer sintaksnog drveta} In essence, syntax trees are part of search space of SR. That is, each sample $\tilde{f}$ is a point in the search space, represented by respective syntax tree. How accurate is this mapping, we compute in the basis of the historical data $D$ and the chosen error   measure (such as $MSE$, or $R^2$). 
...
\section{Proposed iterated local search method}\label{sec:rils}
   ...
   \subsection{Iterated local search framework}

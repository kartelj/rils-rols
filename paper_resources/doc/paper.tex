...
To solve a linear system of $(d+1)$ unknowns and $d+1$ linear equations involved,  Cholesky  factorization, Winograd or Strassen algorithm, just to name a few, may be used  depending on solver of linear equations used as well as the sparsity of the matrix associated to the linear system~\cite{krishnamoorthy2013matrix}. For example, Cholesky decomposition, widely used in solving the problem of linear regression, requires a matrix inversions and matrix multiplications, needs $d^3$ computation cost to find an inverse of the matrix associated to the linear system, where $d$ stands for the number of input features. It is worth to mention that when the number of input features becomes large, these closed-form matrix techniques are usually replaced by iterative ones, such as gradient descent. Using Gradient Descent (GD),  the above complexity reduces to $O(n^2d )$. The learning rate in GD has to be carefully chosen, as it dictates how quick the algorithm is and how fast it converges. Therefore, when comes to input data with large number of features, GS is preferred over OLS.

 
 
%%\fxnote{TODO: ACA: dodati vremensku slozenost za OLS. }%https://towardsdatascience.com/understanding-the-ols-method-for-simple-linear-regression-e0a4e8f692cc

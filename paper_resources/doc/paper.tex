...
\begin{document}
   ...
   The SR researches lack uniform, robust, and transparent
	...
	$$  \frac{\partial}{\partial k_j} \sum_{i}^{n} (\tilde{y}_i - y_i)^2 = \frac{\partial}{\partial k_j} \sum_{i}^{n} ( \mathbf{k}\mathbf{x_i}+b  - y_i)^2 =  \sum_{i}^{n} 2x_{ij}(\mathbf{k}\mathbf{x_i} + b - y_i)  =   . $$
  
	Similarly, by taking into account  partial derivation w.r.t. $b$, we have: 
	$$  \frac{\partial}{\partial b} \sum_{i}^{n} (\tilde{y}_i - y_i)^2  = \frac{\partial}{\partial b} \sum_{i}^{n} ( \mathbf{k}\mathbf{x_i}+b  - y_i)^2 = \sum_{i}^{n} -2  (y_i - \tilde{y}_i). $$
	...
	\begin{align*}
		& \sum_{i}^{n} -2x_{i1} (y_i - \tilde{y}_i) = 0 \\
		& \vdots \\
		& \sum_{i}^{n} -2x_{id} (y_i - \tilde{y}_i) = 0 \\
		&\sum_{i}^{n} -2  (y_i - \tilde{y}_i) = 0.
	\end{align*}
